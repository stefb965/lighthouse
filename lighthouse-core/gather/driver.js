...
class Driver {
  ...
  _checkDomainToggle(domain, action) {
    ...
    if ((enable && newCount === 1) || (!enable && newCount === 0)) {
      log.verbose('Driver', `${domain}.${action}`);
      return true;
    } else {
      if (newCount < 0) {
  ...
  sendCommand(method, params) {
    ...
    if (domainCommand && !this._checkDomainToggle(domainCommand[1], domainCommand[2])) {
  ...
  */
  ...
  beginTrace() {
    ...
    if (this.domainEnabled('Debugger')) {
      log.error('Driver', 'Debugger domain enabled when starting trace');
    }
    if (this.domainEnabled('CSS')) {
      log.error('Driver', 'CSS domain enabled when starting trace');
    }
    if (this.domainEnabled('DOM')) {
      log.error('Driver', 'DOM domain enabled when starting trace');
    }
